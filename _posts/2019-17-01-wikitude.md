...
1. You will not be able to play a Youtube video file in a "magic window" (as seen "attached to the book" in the image above - the lowest blue rectancle with the video icon inside).
...
### Similar books
We wanted to offer related books to the user, and to achieve this we use our own Subject Indexing system


Subject headings...

### More from author
We also wanted a way for a user to find more books by the same author. In our library (as in many other libraries), books are seldom *(ie never)* organized on the shelves by author, rather by some classification system like [Dewey] (https://en.wikipedia.org/wiki/Dewey_Decimal_Classification). In fact, books by the same author might even be spread across several collections, in several rooms.
